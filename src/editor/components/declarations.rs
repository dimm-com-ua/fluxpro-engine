...
#[component]
pub(super) fn EditorTabs(session: Session, active: RwSignal<EditorTab>) -> impl IntoView {
    view! {
            ...
            <main class="fp-declaration-board">
                ...
                <div class="fp-resource-grid">
                    <For each=move || matching.get() key=|id| id.clone() children=move |id| {
                        ...
                        let resource = Memo::new(move |_| session.state.with(|s| s.document.declaration(kind, &id.get_value()).unwrap_or(Value::Null)));
                        let uses = Memo::new(move |_| session.state.with(|s| s.document.declaration_uses(kind, &id.get_value()).len()));
                        view! {
                            <button type="button" class="fp-resource-card" data-resource-kind=kind.singular()
                                ...
                                aria-label=format!("Edit {} {}", kind.singular(), id.get_value()) on:click=move |_| load(id.get_value())>
                ...
                <Show when=move || editing.get() fallback=move || view! {
                    ...
                    <div class="fp-resource-save">
                        <button type="button" class="fp-primary" on:click=move |_| {
                            ...
                            match result {
                                Ok(id) => { load(id); session.message.set(format!("{} saved", kind.singular())); }
...
#[component]
fn JsonField(draft: RwSignal<Value>, pointer: String, label: &'static str) -> impl IntoView {
...
#[component]
fn EscalationActions(draft: RwSignal<Value>, session: Session) -> impl IntoView {
    view! {
