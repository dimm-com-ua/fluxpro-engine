...
fn main() {
    ...
    leptos::mount::mount_to_body(move || {
        ...
        view! {
            <div class:dark=move ||dark.get() style="height:100dvh;padding:16px 24px 24px;background:#edf1f6;box-sizing:border-box;display:flex;flex-direction:column;gap:12px">
                ...
                <nav class="fp-demo-nav" style="display:flex;gap:8px;align-items:center;font:12px -apple-system,sans-serif;color:#68788f">
                    ...
                    <button aria-pressed=move ||dark.get() on:click=move |_|dark.update(|value|*value=!*value)>"Dark theme"</button>
                    <span>{move ||if monitoring.get(){"SIMULATED DATA · 67 instances · updates every 5 seconds"}else{"Embeddable FluxPro components"}}</span>
                </nav>
                <div style="flex:1;min-height:0" hidden=move ||monitoring.get()><ProcessEditor require_applied_changes=true document=document.get_untracked() on_change=Callback::new(move |doc|document.set(doc))/></div>
